...
DESCRIPTION = '''
Модель организационного риска

**Параметры:**

- `ml_model_tag` - если не указан, то берётся значение ml_model_tag из переменных airflow
- `command_path` - базовый путь для всех скриптов внутри docker контейнера

---

Задача: [jira]\
(https://jira.element-lab.ru/browse/KMP-1583)

Вики: [wiki.element-lab]\
(https://wiki.element-lab.ru/pages/viewpage.action?pageId=55777968)
'''
...
ML_MODEL_NAME = 'model_org_risk'


# этапы расчета ML алгоритма
# формат (task_id, входная точка для запуска)
TASKS = [
    ('load_raw_data', 'load_raw_data/main.py'),
    ('process_data', 'process_data/main.py')
]
