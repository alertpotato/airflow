...
@task(outlets=[dataset_scheta_region_period_success])
def update_datasets(ti=None, **context):
    dataset_scheta_region_period_success.extra = {
        "etl_run_id": context["params"]["etl_run_id"],
        "region_cd": context["params"]["region_cd"],
        "period_cd": context["params"]["period_cd"]
    }

    common.log_it({
        "for dataset": dataset_scheta_region_period_success.uri,
        "set params": dataset_scheta_region_period_success.extra
    }, ti.log.info)
...
def dds_etl_scheta():
    set_etl_running_status_task = CustomSQLOperator(
        ...
        conn_id=DQ_CONN_ID,
        sql="ldl_etl_set_running_status_scheta.sql",
        split_statements=True
    )
    ...
    with TaskGroup("check_dq", tooltip="Контроль качества данных") as dq_group:
        ...
        with TaskGroup("check_ffoms", tooltip="Проверка данных по правилам ФФОМС") as dq_ffoms_group:
            get_ffoms_rules_task = PrepareSQLDataQualityOperator(
            ...
            for table_name in SCHETA.keys():
                sql_list_task = get_ffoms_sql.override(task_id=f"get_sql_{table_name}")(table_name,
                                                                                        get_ffoms_rules_task.output)
                sql_list_task.set_upstream(get_ffoms_rules_task)
    ...
    with TaskGroup("report_dq", tooltip="Создание отчета об ошибках") as report_dq_group:
    ...
    >> stage_group >> dq_group >> delete_duplicate_data() >> insert_group >> report_dq_group >> finish_all() >> update_datasets()
