...
builder = yaml_reader.YamlDagBuilder(
    'dataflows/dma/rakurs_smp/dma_etl_rakurs_smp.yaml')

@task
def get_increment_params():
    context = get_current_context()
    sql_op = CustomSQLOperator(
        task_id='get_smp_risk_params',
        conn_id = DMA_CONN_ID,
    ...
    last_update_dttm = result[0][0]
    sql_vars = {
        "smp_risk": last_update_dttm
    }
...
load_dma_rakurs_smp_inc = dma_rakurs_smp_inc()
load_dma_rakurs_smp_inc.template_searchpath.append(path.join(CPP_SCRIPT_PATH))