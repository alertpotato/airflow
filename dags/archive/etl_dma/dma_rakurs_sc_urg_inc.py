...
builder = yaml_reader.YamlDagBuilder(
    'dataflows/dma/rakurs_sc_urg/dma_etl_rakurs_sc_urg.yaml')
...
def get_incr_params():
    ...
    sql_op = CustomSQLOperator(
        task_id='get_sc_urg_risk_params',
        conn_id=DMA_CONN_ID,
...
def dma_rakurs_sc_urg_inc():

    check = check_last_dagrun_completely_successful('dma_etl_cpp_inc')
    builder.create_tasks(
        pre_init_task=check,
        pre_processing_task=get_incr_params()
    )


load_dma_rakurs_sc_urg_inc = dma_rakurs_sc_urg_inc()
load_dma_rakurs_sc_urg_inc.template_searchpath.append(path.join(CPP_SCRIPT_PATH))